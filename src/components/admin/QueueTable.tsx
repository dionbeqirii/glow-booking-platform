...
export default function QueueTable({
  ...
  async function assign(id: string, staffId: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) setError(data.error ?? t("assignFailed"));
      else router.refresh();
    } finally {
      setBusyId(null);
    }
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r) => {
                ...
                const qualifiedStaff = staffOptions.filter((s) => s.serviceIds.includes(r.serviceId));
                                ...
                                <>
                                  ...
                                  {qualifiedStaff.length > 0 && (
                                    <div className="px-3.5 py-1.5">
                                      <label className="block text-[11px] font-medium text-ink-faint">{t("assignStaffLabel")}</label>
                                      <select
                                        value={r.staffId ?? ""}
                                        disabled={busy}
                                        onChange={(ev) => assign(r.id, ev.target.value)}
                                        className="mt-1 w-full rounded-lg border border-line-strong bg-surface px-2 py-1 text-xs text-ink disabled:opacity-50"
                                      >
