...
export default async function ClientsPage() {
  ...
  const [t, locale] = await Promise.all([getTranslations("AdminClients"), getLocale()]);
  const now = new Date();

  const [kpis, rows, topClients] = await Promise.all([
    getClientKpis(now),
    getClientRows(now, locale),
    getTopClients(),
  ]);
        ...
        <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[1fr_260px]">
          <div className="flex h-full min-h-0 min-w-0 flex-col gap-3">
            <div className="shrink-0 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              <Kpi href="/admin/klientet" tone="accent" icon={<IcUsers />} value={kpis.total} label={t("kpiTotal")} sub={t("kpiTotalSub")} />
              <Kpi href="/admin/klientet" tone="gold" icon={<IcUserPlus />} value={kpis.newThisMonth} label={t("kpiNew")} sub={t("kpiNewSub")} />
              <Kpi href="/admin/klientet" tone="ok" icon={<IcPulse />} value={kpis.activeLast30d} label={t("kpiActive")} sub={t("kpiActiveSub")} />
              <Kpi href="/admin/klientet" tone="purple" icon={<IcRepeat />} value={kpis.repeatClients} label={t("kpiRepeat")} sub={t("kpiRepeatSub")} />
            </div>
          ...
          <div className="flex h-full min-h-0 flex-col gap-2 overflow-y-auto">
            <div className="rounded-xl border border-line bg-surface p-2.5">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-faint">{t("segmentsTitle")}</p>
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 text-ink-soft"><span className="h-1.5 w-1.5 rounded-full bg-ok" />{t("segmentActive")}</span>
                  <span className="font-semibold text-ink">{segmentCounts.active}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 text-ink-soft"><span className="h-1.5 w-1.5 rounded-full bg-accent" />{t("segmentNew")}</span>
                  <span className="font-semibold text-ink">{segmentCounts.new}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 text-ink-soft"><span className="h-1.5 w-1.5 rounded-full bg-ink-faint" />{t("segmentInactive")}</span>
            ...
            <div className="rounded-xl border border-line bg-surface p-2.5">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-faint">{t("topClientsTitle")}</p>
              {topClients.length === 0 ? (
                <p className="text-xs text-ink-faint">{t("noBookingsYet")}</p>
              ) : (
                <div className="flex flex-col gap-1.5">
                  {topClients.map((c, i) => (
                    <div key={c.id} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="shrink-0 text-ink-faint">{t("bookingsAbbrev", { count: c.bookingsCount })}</span>
                    </div>
                  ))}
                </div>
              )}
